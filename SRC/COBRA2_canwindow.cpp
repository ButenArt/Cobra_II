...
#include "ui_COBRA2_canwindow.h"

COBRA2_CANWindow::COBRA2_CANWindow(QWidget *parent, T_COBRA2_controls *controls) : QWidget(parent), ui(new Ui::COBRA2_CANWindow)
{
    ui->setupUi(this);
...
COBRA2_CANWindow::~COBRA2_CANWindow()
{
    delete UpdateTimer;
    delete ui;
}

void COBRA2_CANWindow::closeEvent(QCloseEvent *event)
{
    event->ignore();
    UpdateTimer->stop();
    this->hide();
...
void COBRA2_CANWindow::slotParamsUpdate()
{
    {
...
QString COBRA2_CANWindow::Byte2Bin(BYTE val)
{
    QByteArray binValue;
    binValue.insert(0, QString::number(val,2));
    return QString(binValue.data()).rightJustified(8,'0');
}
