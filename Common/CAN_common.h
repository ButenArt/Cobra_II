...
const auto N_MAX_UNITS = 16;           // максимальное количество блоков каждого вида
const auto N_MAX_MAB_UNITS = N_MAX_UNITS;
const auto N_MAX_VID_UNITS = 16;
const auto N_MAX_IN_UNITS = N_MAX_UNITS;
...
struct TENUMData : public TUnitData
{
	explicit TENUMData( size_t i, int addr, int port, int enable );
};
...
public:
	explicit CANBusComm( QString sboard = "MOXA" );
	~CANBusComm( );
	void Start( );
	void Stop( );
	T_SOD SOD;
...
protected:
	...
	BYTE Hi( WORD value );
	BYTE Lo( WORD value );
	WORD MAB_VALUE( BYTE byte_Hi, BYTE byte_Lo, WORD max_val );
	WORD MAKE_WORD( BYTE byte_Hi, BYTE byte_Lo );
	USHORT ENUM_VALUE( BYTE byte_Hi, BYTE byte_Lo );
private slots:
};
