...
{
	// qDebug( ) << __FUNCTION__;
	assert(param_id >= 0 && param_id < PARAM_IN.size());
...
// при этом делает кучу не нужной работы, если не пишешь 0, то зачем счиать дальше?????
//TODO переписать для записи 0
void T_Controls::SetGPIOOUTBitValue(bool value, BYTE param_id)
{
	// qDebug( ) << __FUNCTION__;
	assert(param_id >= 0 && param_id < PARAM_OUT.size());
...
{
	// qDebug( ) << __FUNCTION__;
	assert( param_id >= 0 && param_id < PARAM_ENC.size( ) );
...
{
	// qDebug( ) << __FUNCTION__;
	assert( param_id >= 0 && param_id < PARAM_POT.size( ) );
...
{
	// qDebug( ) << __FUNCTION__;
	assert( param_id >= 0 && param_id < PARAM_LED.size( ) );
...
void T_Controls::SetLEDINDICATORValue( qint32 value, BYTE param_id )
{
	...
	if ( value != 65535 )
	{
		...
		{
			if ( outval.at( i ).isDigit( ) ) can->SOD.LED_UNIT[unitnum].DIGIT[i] = outval.at( i ).digitValue( );
		}
	}
}
...
{
	// qDebug( ) << __FUNCTION__;
	assert( param_id >= 0 && param_id < PARAM_LCD.size( ) );
