...
CANBusComm::CANBusComm( QString sboard )
{
...
// Lower byte
BYTE CANBusComm::Lo( WORD value )
{
	BYTE val = value & 0xFF;
	return val;
}

WORD CANBusComm::MAB_VALUE( BYTE byte_Hi, BYTE byte_Lo, WORD max_val )
{
	WORD ret = ( ( byte_Hi << 8 ) | byte_Lo & 0x00FF );
	return max_val > 1023 ? ret : ret >> 2;
}

WORD CANBusComm::MAKE_WORD( BYTE byte_Hi, BYTE byte_Lo )
{
	return ( ( byte_Hi << 8 ) | byte_Lo & 0x00FF );
}

USHORT CANBusComm::ENUM_VALUE( BYTE byte_Hi, BYTE byte_Lo )
{
...
TMABData::TMABData( size_t i, int addr, int port, int enable )
{
	index = i;
...
TVIDData::TVIDData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x4;
	ADDR_RX = addr + 0xF;
...
TGPIOINData::TGPIOINData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0;
	ADDR_RX = addr + 0x9;
...
TGPIOOUTData::TGPIOOUTData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x1;
	ADDR_RX = addr + 0xF;
...
TPOTData::TPOTData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x2;
	ADDR_RX = addr + 0xA;
...
TENCData::TENCData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x0;
	ADDR_RX = addr + 0x9;
...
TLEDData::TLEDData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0;
	ADDR_RX = addr + 0x9;
...
TENUMData::TENUMData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x03;
	ADDR_RX = addr + 0x0C;
	...
	INFO = "";
	auto number = QString::number( i );
	DESC = "ENUM" + number + " - Addr TX/RX: [0x" + QString::number( ADDR_TX, 16 ).toUpper( ) + "]/[0x" + QString::number( ADDR_RX, 16 ).toUpper( ) + "] Port : [" + QString::number( PORT ) + "] Enabled : [" + QString::number( ENABLE ) + "] ";

	sFAULT = "ENUM" + number + ": [0x" + QString::number( ADDR_TX, 16 ).toUpper( ) + "] Port: [" + QString::number( PORT ) + "] \r\n";
}

TLCDData::TLCDData( size_t i, int addr, int port, int enable )
{
	index = i;
	ADDR_TX = addr + 0x03;		// TODO узнать и устаровить реальные адреса для LCD
	ADDR_RX = addr + 0x0C;
	PORT = port;
	ENABLE = enable;
	TX = 0;
	...
	sFAULT = "ECL" + number + ": [0x" + QString::number( ADDR_TX, 16 ).toUpper( ) + "] Port: [" + QString::number( PORT ) + "] \r\n";
}